# EXAMPLE 3: 12 LATENT VARIABLES, ALL TRIPLETS, CLUSTERS OF FOUR TRIPLETS (EACH NODE CONNECTED TO THREE OTHER TRIPLETS)
...
#CASE - CUSTOM
#ecov <- c(0.22, -0.15, -0.39, 0.22, -0.15, -0.39, 0.22, -0.15, -0.39, 0.22, -0.15, -0.39)

#CASE - RANDOM 
#ecov = sample(ii_choice, 12, replace = TRUE)
...
pop.model <- c("
    ...
    # model K
    K  =~ (", l1, ")*K.x + (", l2, ")*K.y + (", l3, ")*K.z
    K.bf =~ (", K.l2s, ")*K.y + (", K.l3s, ")*K.z
    ...
    # model L
    L  =~ (", l1, ")*L.x + (", l2, ")*L.y + (", l3, ")*L.z
    L.bf =~ (", L.l2s, ")*L.y + (", L.l3s, ")*L.z
...
df_ii <- data.frame(ii_score=as.numeric(12),
                    ii_programmed=character(12),
